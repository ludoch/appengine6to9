...
public class BackendsYamlReader {
  ...
  public static class BackendsYaml {
    ...
    // YamlBeans uses Bean Introspection.  We cannot create a
    // getClass() method to retrieve the YAML 'class' property, so we
    // want to tell Beans Introspection to call getInstanceClass()
    // instead.  Luckily, Beans will first look for a BeanInfo (by
    // appending 'BeanInfo' to the class name) before reverting to
    // reflection.
